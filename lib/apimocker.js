...
apiMocker.sendResponse = function(req, res, next, serviceKeys) {
  ...
  setTimeout(function() {
	...
	if(options.enableTemplate === true){
      ...
      fs.access(path.join(apiMocker.options.mockDirectory, options.mockFile), fs.R_OK, function(err) {
        // pass requests to proxy if necessary
        if (err) {
          next();
        }
        else {
...
apiMocker.setRoute = function(options) {
  ...
  apiMocker.router[options.verb]("/" + options.serviceUrl, function(req, res, next) {

    apiMocker.sendResponse(req, res, next, options);
  });
